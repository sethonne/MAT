...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    plot(NA,
      xlim = c(min(pts$x), max(pts$x)), ylim = c(-1, 9),
    ...
    # Sun
    symbols(min(pts$x) + 1.5, 7.5,
      circles = 0.6, inches = FALSE,
    ...
    for (wox in c(-car_w / 2 + 0.3, car_w / 2 - 0.3)) {
      ...
      symbols(w[1], w[2],
        circles = wheel_r, inches = FALSE, add = TRUE,
        bg = "#1a1a1a", fg = "black"
      )
      symbols(w[1], w[2],
        circles = wheel_r * 0.4, inches = FALSE, add = TRUE,
        bg = "#888", fg = "#444"
      )
    }
    ...
    symbols(hl[1], hl[2],
      circles = 0.07, inches = FALSE, add = TRUE,
      bg = "#FFF7AE", fg = "black"
    )
  })
}
